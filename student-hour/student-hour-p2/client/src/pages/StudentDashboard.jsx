  ...
  return (
    <div style={{ width: 240, background: 'var(--brown-900)', display: 'flex', flexDirection: 'column', height: '100vh', flexShrink: 0 }}>
      <div style={{ padding: '28px 24px 20px' }}>
        ...
        <div style={{ fontSize: 11, color: 'var(--brown-400)', marginBottom: 3, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Welcome back</div>
      ...
      <nav style={{ flex: 1, padding: '0 12px' }}>
        ...
        <div style={{ marginTop: 8, borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: 8 }}>
          <Link to="/dashboard/nova" style={{
            display: 'block', padding: '11px 14px', borderRadius: 'var(--radius-md)', fontSize: 14,
            background: 'rgba(245,200,66,0.08)', color: 'var(--yellow-300)', textDecoration: 'none',
            border: '1px solid rgba(245,200,66,0.2)', textAlign: 'left'
          }}>
            Professor Nova ✦
          </Link>
        </div>
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--surface)' }}>
      ...
      <main style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '14px 28px', borderBottom: '1px solid var(--border-soft)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#fff', minHeight: 56 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <span style={{ fontWeight: 600, fontSize: 15, color: 'var(--brown-900)' }}>
              {active === 'chat' ? group?.name || 'Group Chat' : active === 'tasks' ? 'My Tasks' : active === 'resources' ? 'Resources' : 'Study Schedule'}
            </span>
...
function TasksView({ groupId }) {
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 700, overflowY: 'auto', height: '100%' }}>
      {tasks.length === 0
        ...
        : resources.map(r => (
          <div key={r.id} className="card" style={{ marginBottom: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
              <div style={{ fontWeight: 600, fontSize: 15, color: 'var(--brown-900)' }}>{r.title}</div>
              {r.for_nova && <span className="badge badge-yellow" style={{ fontSize: 11 }}>Fed to Prof Nova</span>}
            </div>
            {r.content_text && <p style={{ fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.7 }}>{r.content_text.slice(0, 300)}{r.content_text.length > 300 ? '...' : ''}</p>}
...
function PlaceholderView({ text }) {
  return <div style={{ padding: '40px 32px', color: 'var(--text-muted)', fontSize: 14 }}>{text}</div>
}
