    ...
    <div style={{ minHeight: '100vh', background: 'var(--surface)' }}>
      ...
      <nav style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 40px', borderBottom:'1px solid var(--border-soft)', background:'rgba(253,250,247,0.95)', backdropFilter:'blur(12px)', position:'sticky', top:0, zIndex:100 }}>
        ...
        <div className="nav-desktop" style={{ display:'flex', gap:12, alignItems:'center' }}>
          {dashLink ? (
            <>
              ...
              <Link to={dashLink} className="btn-primary" style={{ padding:'9px 22px', fontSize:14 }}>
                Go to dashboard →
              </Link>
            </>
          ) : (
        ...
        <div className="nav-mobile-cta" style={{ display:'none', gap:10, alignItems:'center' }}>
          {dashLink ? (
            <Link to={dashLink} className="btn-accent" style={{ padding:'9px 18px', fontSize:13 }}>Dashboard →</Link>
          ) : (
      ...
      {menuOpen && (
        <div style={{ background:'#fff', borderBottom:'1px solid var(--border-soft)', padding:'12px 20px 16px', display:'flex', flexDirection:'column', gap:10 }}>
          {dashLink
            ? <Link to={dashLink} className="btn-primary" style={{ width:'100%', justifyContent:'center', display:'flex' }} onClick={() => setMenuOpen(false)}>Go to dashboard →</Link>
        ...
        <div className="hero-grid" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:56, alignItems:'center' }}>
          <div>
            ...
            <div style={{ display:'flex', gap:14, flexWrap:'wrap' }}>
              {dashLink ? (
                <Link to={dashLink} className="btn-accent" style={{ fontSize:16, padding:'14px 30px' }}>Continue learning →</Link>
              ) : (
                <>
                  <Link to="/signup" className="btn-accent" style={{ fontSize:16, padding:'14px 30px' }}>Start learning free</Link>
                  <Link to="/login" className="btn-outline" style={{ fontSize:16, padding:'14px 30px' }}>Log in</Link>
                </>
              )}
            </div>
          </div>
            ...
            <div style={{ background:'#fff', borderRadius:'var(--radius-lg)', padding:'18px 20px', border:'1px solid var(--border-soft)', marginBottom:16 }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:11, color:'var(--text-muted)' }}>Your AI tutor · online</div>
                </div>
              </div>
      ...
      <section id="how-it-works" style={{ padding:'60px 24px', background:'var(--surface-2)' }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          ...
          <div className="steps-grid" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:18 }}>
            {[
              ['01','Sign up & share your weak spots','Enter your course codes. Takes 2 minutes.'],
              ['02','Get grouped by AI','Smart matching with students facing the same challenges.'],
              ['03','Your mentor steps in','A human mentor guides your group and shares resources.'],
              ['04','Learn with Professor Nova','Your AI tutor builds on every session.'],
            ].map(([s,t,d]) => (
        ...
        <div className="nova-grid" style={{ maxWidth:1100, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 1fr', gap:60, alignItems:'center' }}>
          <div>
            ...
            <p style={{ fontSize:15, color:'var(--text-secondary)', lineHeight:1.8, marginBottom:20 }}>
              Not a chatbot. He knows your history, adapts to your pace, uses humor, and never makes you feel judged for not knowing.
            </p>
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              {[
                'Remembers your name, courses, and past struggles',
                'Teaches in classroom mode or one-on-one personal mode',
                'Uses mentor-uploaded materials as his primary source',
                'Builds a private learning profile that grows every session',
              ].map(f => (
          ...
          <div style={{ background:'var(--brown-900)', borderRadius:'var(--radius-xl)', padding:32 }}>
            ...
            <p style={{ color:'var(--brown-200)', fontSize:14, lineHeight:1.8, fontStyle:'italic', marginBottom:18 }}>
              "I exist to make you understand — not just memorize. If one approach isn't landing, we try another. I don't give up on you."
            </p>
      ...
      <section style={{ padding:'60px 24px', background:'var(--brown-900)', textAlign:'center' }}>
        ...
        <h2 style={{ fontFamily:'var(--font-serif)', fontSize:'clamp(24px,4vw,42px)', fontWeight:700, color:'#fff', marginBottom:14 }}>
          Your breakthrough semester starts here
        </h2>
        <p style={{ fontSize:15, color:'var(--brown-300)', marginBottom:32, maxWidth:460, margin:'0 auto 32px' }}>
          Join students who stopped falling behind and started catching up.
        </p>
        {dashLink
          ? <Link to={dashLink} className="btn-accent" style={{ fontSize:16, padding:'14px 36px' }}>Go to your dashboard →</Link>
