...
//
// 1. MOBILE DEAF — Android Chrome ignores continuous:true silently
//    and also requires a user gesture before mic opens.
//    Fix: on mobile, use single-shot mode that restarts after each
//    result. Also added explicit mic permission request on mount.
//
// 2. AVATAR/VOICE SYNC — avatar was set to 'speaking' before TTS
//    started (during waitForVoices delay). Mouth moved, no sound.
//    Fix: set novaState='speaking' only INSIDE speak() after the
//    first utterance actually starts (onstart event).
//    Text board appears immediately. Avatar syncs to actual audio.
//
// 3. INTERRUPTION — VAD arm() was in wrong place / never called
//    Fix: clean two-system design. SpeechRecognition always runs.
//    VAD watches volume. When your voice detected → Nova cut off
//    → 150ms settle → your words arrive via STT → sendMessage.
// ─────────────────────────────────────────────────────────────────

const ON_MOBILE = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent)

// ─────────────────────────────────────────────────────────────────
// NOVA EAR — always running, never paused during Nova's speech
// Desktop: continuous session. Mobile: single-shot with restart.
// ─────────────────────────────────────────────────────────────────
class NovaEar {
  ...
  // Brief window after TTS cut — ignores residual speaker echo
  settle(ms = 200) {
    this.settling = true
    clearTimeout(this.settleT)
    this.settleT = setTimeout(() => { this.settling = false }, ms)
  }
  ...
  _start() {
    ...
    // Mobile: single-shot (continuous silently ignored on Android)
    // Desktop: continuous so it never dies between phrases
    rec.continuous = !ON_MOBILE

    rec.onresult = (e) => {
      if (this.settling) return // residual echo window — drop
      let best = '', bestConf = -1
    ...
    rec.onerror = (e) => {
      ...
      // On mobile, 'no-speech' is common — restart quickly
      const delay = e.error === 'no-speech' ? 100 : 600
      if (this.running) this.restartT = setTimeout(() => this._start(), delay)
    }

    rec.onend = () => {
      if (!this.running) return
      // Always restart — critical for mobile single-shot mode
      this.restartT = setTimeout(() => this._start(), ON_MOBILE ? 150 : 400)
    }

    this.rec = rec
    try { rec.start() }
    catch (_) {
      if (this.running) this.restartT = setTimeout(() => this._start(), 500)
    }
  }
}

// ─────────────────────────────────────────────────────────────────
// VAD — AudioContext volume detector for barge-in interruption
// Completely separate from SpeechRecognition — no echo conflict
// ─────────────────────────────────────────────────────────────────
class VAD {
  constructor(onVoice) {
    this.onVoice = onVoice
    this.ctx = null; this.analyser = null; this.stream = null
    this.rafId = null; this.armed = false; this.fired = false
    this.holdStart = null
    this.THRESHOLD = 14 // RMS 0-255. Lower = more sensitive for mobile
    this.HOLD_MS = 80   // 80ms sustained voice to trigger
  }
  ...
  _loop() {
    ...
    const tick = () => {
      ...
      if (rms > this.THRESHOLD) {
        ...
        else if (Date.now() - this.holdStart >= this.HOLD_MS) {
          this.fired = true; this.armed = false
          this.onVoice()
        }
...
// ─────────────────────────────────────────────────────────────────
// TTS — waits for voices, syncs avatar to actual audio start
// ─────────────────────────────────────────────────────────────────
function getVoice() {
  ...
  return voices.find(v =>
    v.lang?.startsWith('en') && !/(female|zira|hazel|victoria|karen|samantha)/i.test(v.name)
...
function waitForVoices() {
  return new Promise(resolve => {
    ...
    let done = false
    const h = () => { if (!done) { done = true; resolve() } }
    window.speechSynthesis.onvoiceschanged = h
    setTimeout(() => { if (!done) { done = true; resolve() } }, 3000)
  })
}

// onStart fires when FIRST utterance actually begins playing
// onDone fires when all sentences finished
// cancelRef: speakingRef — when false, stop chain
function speak(text, { onStart, onDone, cancelRef } = {}) {
  ...
  waitForVoices().then(() => {
    ...
    function next() {
      ...
      u.onstart = () => {
        if (!started) { started = true; onStart?.() } // avatar sync here
      }
      u.onend = next
      u.onerror = () => next()
...
export default function ProfessorNovaPage() {
  ...
  const interrupt = useCallback(() => {
    ...
    earRef.current?.settle(200) // let speaker audio clear
  }, [])
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      ...
      catch (_) { throw new Error('Bad response from server — try again.') }

      const reply = data.reply
      setMessages(prev => [...prev, { role: 'assistant', content: reply }])
      // Show text immediately — avatar stays 'thinking' until audio starts
      setBoardText(reply)
      setBoardVisible(true)

      if (voiceOnRef.current) {
        speakingRef.current = true
        // Avatar stays 'thinking' here — syncs to audio in onStart
        vadRef.current?.arm()
        speak(reply, {
          onStart: () => {
            // Audio actually started — NOW animate the avatar
            if (speakingRef.current) setNovaState('speaking')
          },
          onDone: () => {
            speakingRef.current = false
            vadRef.current?.disarm()
  ...
  useEffect(() => {
    ...
    // Request mic permission early (critical on mobile)
    navigator.mediaDevices?.getUserMedia({ audio: true })
      .then(stream => {
        // Got permission — stop this stream, VAD will open its own
    ...
    const t = setTimeout(() => {
      ...
      if (voiceOnRef.current && !greetedRef.current) {
        ...
        speakingRef.current = true
        speak(
          'Hello ' + name + '. I am Professor Nova. You can interrupt me any time — just speak.',
          {
  ...
  return (
    <div style={{ height:'100vh', background:'#080604', display:'flex', flexDirection:'column', overflow:'hidden', fontFamily:'sans-serif', position:'relative' }}>
      ...
      <div style={{ padding:'10px 14px', display:'flex', alignItems:'center', justifyContent:'space-between', background:'rgba(0,0,0,0.8)', backdropFilter:'blur(20px)', borderBottom:'1px solid rgba(255,255,255,0.04)', flexShrink:0, zIndex:50 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:9, textTransform:'uppercase', letterSpacing:'0.07em', color: novaState==='speaking'?'#22c55e':novaState==='thinking'?'#f5c842':micOn?'rgba(100,200,255,0.5)':'rgba(255,255,255,0.25)' }}>
              {novaState==='speaking'?'speaking — interrupt any time':novaState==='thinking'?'thinking…':micOn?'listening · session '+sessionCount:'session '+sessionCount}
            </div>
          </div>
        </div>
        <div style={{ display:'flex', gap:6, alignItems:'center' }}>
          ...
          <button className="nvbtn" onClick={() => {
            const v = !voiceOn; setVoiceOn(v); voiceOnRef.current = v
            if (!v && speakingRef.current) { window.speechSynthesis.cancel(); speakingRef.current=false; vadRef.current?.disarm(); setNovaState('idle') }
      ...
      <div style={{ flex:1, position:'relative', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
        ...
        {!boardVisible && messages.length===0 && novaState==='idle' && (
          <p style={{ fontSize:12, color:'rgba(255,255,255,0.2)', marginTop:8, zIndex:10 }}>
            {micOn ? 'Speak naturally — interrupt me any time' : 'Tap Mic On to talk'}
          </p>
        )}
      ...
      <div style={{ position:'absolute', bottom:16, left:0, right:0, display:'flex', justifyContent:'center', alignItems:'center', gap:14, zIndex:50 }}>
        ...
        {novaState==='speaking' && (
          <button className="nvbtn" onClick={interrupt} style={{ width:46, height:46, borderRadius:'50%', background:'rgba(220,38,38,0.15)', border:'1px solid rgba(220,38,38,0.3)', fontSize:11, color:'#fca5a5', backdropFilter:'blur(12px)' }}>
            Stop
          </button>
        )}
      </div>
