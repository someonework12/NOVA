...
export default function OnboardingPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (!valid.length) { setError('Add at least one course.'); setLoading(false); return }

    const { error: pe } = await supabase.from('profiles').upsert({ id: user.id, department, university, role: 'student', onboarding_complete: true })
    if (pe) { setError(pe.message); setLoading(false); return }

    const { error: ce } = await supabase.from('student_courses').insert(
      valid.map(c=>({ student_id: user.id, course_code: c.code, course_title: c.title, weakness_description: c.weakness }))
    )
    ...
    <div style={{ minHeight: '100vh', background: 'var(--surface)', padding: '40px 16px' }}>
      <div style={{ maxWidth: 640, margin: '0 auto' }}>
        <div style={{ marginBottom: 36 }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(24px,5vw,32px)', fontWeight: 700, color: 'var(--brown-900)', marginBottom: 10 }}>Tell us where you need help</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: 15, lineHeight: 1.7 }}>This helps us group you and build your Professor Nova profile.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
              ...
              {courses.length < 6 && (
                <button type="button" onClick={addCourse} className="btn-outline" style={{ padding: '7px 14px', fontSize: 13 }}>+ Add course</button>
              )}
            </div>
            {courses.map((course, i) => (
              <div key={i} style={{ background: 'var(--surface-2)', borderRadius: 'var(--radius-md)', padding: 16, position: 'relative' }}>
                {courses.length > 1 && (
                  <button type="button" onClick={()=>removeCourse(i)} style={{ position: 'absolute', top: 12, right: 14, background: 'none', color: 'var(--text-muted)', fontSize: 20, cursor: 'pointer', lineHeight: 1 }}>×</button>
                )}
                <div className="course-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 10, marginBottom: 10 }}>
                  <div>
                    <label className="label">Course code</label>
                    <input className="input-field" placeholder="MTH301" value={course.code} onChange={e=>updateCourse(i,'code',e.target.value)} required />
                  </div>
                  <div>
                    <label className="label">Course title</label>
                    <input className="input-field" placeholder="Real Analysis" value={course.title} onChange={e=>updateCourse(i,'title',e.target.value)} required />
                  </div>
                </div>
                <div>
                  <label className="label">What are you struggling with? (optional)</label>
                  <input className="input-field" placeholder="e.g. Integration techniques, proofs..." value={course.weakness} onChange={e=>updateCourse(i,'weakness',e.target.value)} />
                </div>
              </div>
            ))}
          </div>
