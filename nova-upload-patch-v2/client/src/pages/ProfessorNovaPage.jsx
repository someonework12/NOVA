...
export default function ProfessorNovaPage() {
  ...
  async function handlePdfUpload(e) {
    ...
    if (file.size > 40 * 1024 * 1024) {
      setPdfMsg('File too large. Maximum size is 40 MB.')
      return
    }
    setPdfUploading(true)
    setPdfMsg('Uploading… this may take a minute for large files.')
    ...
    try {
      ...
      // AbortController gives us a 120s timeout so large PDFs don't silently 504
      ...
      const charCount = data.chars ? ` (${(data.chars/1000).toFixed(0)}k chars read)` : ''
      setPdfMsg(`"${file.name}" uploaded successfully${charCount}. Professor Nova will use this when teaching you.`)
      // Reload materials list
      const { data: mats } = await supabase.from('nova_materials')
        .select('id, file_name, created_at, chars')
        .eq('student_id', profile.id)
        .order('created_at', { ascending: false })
      setMaterials(mats || [])
    } catch (err) {
        ...
        <div style={{ position:'absolute', inset:0, zIndex:110, background:'rgba(0,0,0,0.88)', backdropFilter:'blur(20px)', display:'flex', flexDirection:'column', animation:'chat-in 0.25s ease-out' }}>
          ...
          <div style={{ flex:1, overflowY:'auto', padding:'16px' }}>
            ...
            <div style={{ border:'2px dashed rgba(100,200,120,0.3)', borderRadius:12, padding:'24px', textAlign:'center', marginBottom:16, cursor:'pointer', background:'rgba(100,200,120,0.04)' }} onClick={() => fileInputRef.current?.click()}>
              ...
              <div style={{ fontSize:11, color:'rgba(255,255,255,0.25)' }}>PDF, Word (.docx), or plain text — max 40 MB · 1000+ pages supported</div>
            </div>
