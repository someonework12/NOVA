...
// ─────────────────────────────────────────────────────────────────
// ARCHITECTURE — clean full-duplex barge-in design
//
// Two completely separate systems run in parallel:
//
// 1. NovaEar (SpeechRecognition)
//    - Always running when mic is on
//    - NEVER paused during Nova's speech
//    - While Nova speaks: results go to interruptHandler
//    - While Nova is quiet: results go to sendMessage
//
// 2. VAD (AudioContext volume detector)
//    - Watches raw mic volume continuously
//    - When volume spike detected during Nova's speech → interrupt
//    - Fires BEFORE SpeechRecognition delivers a final result
//    - Cuts Nova off in ~120ms — feels instant
//
// Interruption flow:
//   You speak → VAD fires (~120ms) → Nova's TTS cancelled
//             → SpeechRecognition delivers your words (~500ms)
//             → sendMessage runs with your actual words
//
// Echo prevention:
//   While Nova speaks, SpeechRecognition stays ON (needed for
//   barge-in) but results are held. VAD triggers the cancel.
//   After TTS cancel, 150ms settle before accepting STT results.
//   This is enough time for speaker audio to clear from the mic.
// ─────────────────────────────────────────────────────────────────

const ON_MOBILE = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent)

// ─────────────────────────────────────────────────────────────────
// NOVA EAR — speech recognition, always alive
// ─────────────────────────────────────────────────────────────────
class NovaEar {
  constructor(onSpeech) {
    ...
    this.restartT = null
    this.settleT = null
    this.settling = false  // true for 150ms after TTS cancelled — ignore results
  }
  ...
  // Called right after TTS is cancelled — brief settle window
  // so residual speaker audio doesn't get sent as a message
  settle(ms = 150) {
    this.settling = true
  ...
  _start() {
    ...
    rec.onresult = (e) => {
      // During settle window — residual speaker audio, ignore
    ...
    rec.onerror = (e) => {
      ...
      if (this.running) this.restartT = setTimeout(() => this._start(), 600)
    }

    rec.onend = () => {
      if (!this.running) return
      this.restartT = setTimeout(() => this._start(), ON_MOBILE ? 200 : 400)
    }
...
// ─────────────────────────────────────────────────────────────────
// VAD — voice activity detector via AudioContext
// Fires onVoice() when your voice energy is detected
// Works independently of SpeechRecognition
// ─────────────────────────────────────────────────────────────────
class VAD {
  constructor(onVoice) {
    ...
    this.fired = false
    this.holdStart = null
    this.THRESHOLD = 16  // RMS 0–255. 16 catches clear speech, ignores breath/noise
    this.HOLD_MS = 100   // sustain for 100ms to avoid click triggers
  }
...
// ─────────────────────────────────────────────────────────────────
// TTS — waits for voices, splits into sentences, deep voice
// ─────────────────────────────────────────────────────────────────
function getVoice() {
...
// cancelRef: speakingRef — when it flips to false, next() stops
...
export default function ProfessorNovaPage() {
  ...
  // ── INTERRUPT — cuts Nova off, ear settles, then listens ─────
  const interrupt = useCallback(() => {
    ...
    // Settle: 150ms to let speaker audio clear before accepting STT
    earRef.current?.settle(150)
  }, [])
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    // If Nova is speaking, cut her off
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 504) msg = 'Professor Nova is waking up — please try again in a moment.'
      ...
      if (voiceOnRef.current) {
        ...
        // Arm VAD so your voice triggers interrupt while Nova speaks
        vadRef.current?.arm()
        speak(reply, () => {
          // Nova finished naturally (not interrupted)
          speakingRef.current = false
          vadRef.current?.disarm()
          setNovaState('idle')
          setBoardVisible(false)
  ...
  useEffect(() => {
    ...
    // Ear — always running, passes speech to sendMessage
    const ear = new NovaEar((text) => {
      if (sendRef.current) sendRef.current(text)
    })
    earRef.current = ear

    // VAD — fires when your voice detected while Nova speaks
    const vad = new VAD(() => {
      // Your voice detected — cut Nova off immediately
      // Ear is still running — your actual words arrive via STT shortly after
      if (speakingRef.current) {
        ...
        setNovaState('idle')
        setBoardVisible(false)
        ear.settle(150)  // brief settle so TTS audio clears before STT result arrives
      }
    })
    vadRef.current = vad
    vad.init()

    // Wake Render on load, keep alive every 13 min
    ...
    const t = setTimeout(() => {
      ...
      if (voiceOnRef.current && !greetedRef.current) {
        ...
        setNovaState('speaking')
        speak(
          'Hello ' + name + '. I am Professor Nova. Speak to me naturally — you can interrupt me any time.',
          () => {
            speakingRef.current = false
            setNovaState('idle')
            ear.open()
